...
#' 
#' @examples
#' listOutMix <- sensiAnPaperPart( 3, insecticideUsed = 'mixture' )
#' listOutI1 <- sensiAnPaperPart( 3, insecticideUsed = 'insecticide1' )
#' listOutI2 <- sensiAnPaperPart( 3, insecticideUsed = 'insecticide2' )
...
#' @export
#' 
sensiAnMosaic <- function( nScenarios = 5000,
                           ...
                           old_insecticide = FALSE )
{
  ...
  for(i in 1:nScenarios)
  {
    ...
    } else
    {
      ...
      # 30th / 11 /2020 set this up for panel nets: 0.2-0.8 discrete
      
      # Is 0?
      
      #correct_mix_deploy <- 0     
      
      correct_mix_deploy<-sample(seq(0.2,0.8,by=0.1),size=1,replace=TRUE)
    }
...
# And save listout as an .rda: 
save(listOut, file= "D:/SAM/Documents/V2_Resistance/Resistance/Data/SF_Panel30_5K.rda")
